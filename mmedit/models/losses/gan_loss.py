        ...
        if self.gan_type == 'hinge':
            ...
            if is_disc:
                if target_is_real:
                    ...
                    target_label = self.gaussian_blur(
                        mask).detach().cuda()
...
class GaussianBlur(nn.Module):
    """A Gaussian filter which blurs a given tensor with a two-dimensional gaussian kernel 
    by convolving it along each channel. Batch operation is supported.
    ...
    Args:
        ...
        sigma (tuple[float]): The standard deviation of the kernel. Default (10.0, 10.0)
    ...
    def get_2d_gaussian_kernel(self, kernel_size, sigma):
        ...
        Args:
            kernel_size (tuple[int]): Kernel filter size in the x and y direction. 
                                      The kernel sizes should be odd and positive.
            sigma (tuple[int]): Gaussian standard deviation in the x and y
                                direction.
        
        Returns:
            kernel_2d (Tensor): A 2D torch tensor with gaussian filter matrix coefficients.
    ...
    def get_1d_gaussian_kernel(self, kernel_size, sigma):
        ...
        Returns:
            kernel_1d (Tensor): A 1D torch tensor with gaussian filter coefficients in x or y direction.
    ...
    def gaussian(self, kernel_size, sigma):
        gauss_arg = lambda x: -(x - kernel_size // 2) ** 2 / float(2 * sigma ** 2)
        gauss = torch.stack(
